...
class Family(object):
    ...
    def cousin(self, aName, bName):
        ...
        cousintype = 0
        degreesremoved = 0
        a_node = self.names_to_nodes[aName]
        b_node = self.names_to_nodes[bName]
        if aName == bName:
            cousintype = -1
            degreesremoved = 0
        else:
            aParent = []
            bParent = []
            a_parent = a_node # add a new line
            while True:
                aParent.insert(0, str(a_parent))                
                if str(a_parent) == str(self.root):
                    break
                else:
                    a_parent = a_parent.get_parent()  # change from a_parent = a_node.get_parent() to a_parent = a_parent.get_parent()
            b_parent = b_node # add a new line
            while True:
                bParent.insert(0, str(b_parent))
                if str(b_parent) == str(self.root):
                    break
                else:
                    b_parent = b_parent.get_parent() # change from b_parent = b_node.get_parent() to b_parent = b_parent.get_parent()
            #print aParent
            #print bParent
            short = min(len(aParent), len(bParent))
            k = -1
            for i in range(short):
                if aParent[i] != bParent[i]:
                    cousintype = short - i - 1
                    k = i
                    break
            if (k == -1):
                cousintype = -1
            degreesremoved  = abs(len(aParent) - len(bParent))
