...
def DFSBinary(root, fcn):
    ...
    while len(queue) > 0:
        ...
        if fcn(queue[0]):
            ...
            temp = queue.pop(0)
            if temp.getRightBranch(): #if right branch exists
...
def BFSBinary(root, fcn):
    ...
    while len(queue) > 0:
        print 'at node ' + str(queue[0].getValue())
        if fcn(queue[0]): #queue[0] is a node
            return True
        else:
            temp = queue.pop(0)
            if temp.getLeftBranch():
...
def DFSBinaryPath(root, fcn):
    queue = [root] #treeTest is the root
    while len(queue) > 0:
        if fcn(queue[0]):
...
def TracePath(node):
    if not node.getParent(): #if this node does not have a parent, then the root is itself
        return [node]
    else:
...
def buildDTree(sofar, todo):
    if len(todo) == 0:
        print "binaryTree(sofar)"
        return binaryTree(sofar)
    else:
        print "withelt = buildDTree(sofar + [todo[0]], todo[1:])"
        withelt = buildDTree(sofar + [todo[0]], todo[1:]) 
        print "withoutelt = buildDTree(sofar, todo[1:])"
        withoutelt = buildDTree(sofar, todo[1:])
        print "here = binaryTree(sofar)"
        here = binaryTree(sofar) #create a node
        here.setLeftBranch(withelt)
        here.setRightBranch(withoutelt)
        return here  #return a tree
...
def DTImplicit(toConsider, avail):
    if toConsider == [] or avail == 0:
        result = (0, ())    #
    elif toConsider[0][1] > avail: #a[], b[], c[], d[] are lists.
        ...
        if withVal > withoutVal:
            result = (withVal, withToTake + (nextItem,)) #when calculated withoutVal, for example, a was not included, so in result, a should be added back
        else:
...
val, taken = DTImplicit(stuff, 10) # 10 is available size
